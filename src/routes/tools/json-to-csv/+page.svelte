<script lang="ts">
  ...
  },
  {
	...
	async function convert() {
		...
		try {
			const response = await fetch('/api/tools/json-to-csv', {
				method: 'POST',
				headers: { 'Content-Type': 'application/json' },
				body: JSON.stringify({ jsonData: jsonData.trim(), delimiter })
			});
...
<div class="container mx-auto px-4 py-8 space-y-6">
	...
	<div class="space-y-2">
		<div class="flex items-center gap-3">
			...
			<h1 class="text-4xl font-bold">JSON to CSV Converter</h1>
		</div>
	...
	<div class="grid md:grid-cols-2 gap-6">
		...
		<div class="space-y-4">
			...
			>
				<label class="label">
					<span class="font-semibold mb-2 block">JSON Input</span>
					<textarea
						bind:value={jsonData}
			...
			{:else if result}
				...
				>
					<div class="flex items-center justify-between">
						<div>
							<h2 class="text-xl font-bold">CSV Output</h2>
							<p class="text-sm text-surface-600 dark:text-surface-400">
								{rowCount}
								{rowCount === 1 ? 'row' : 'rows'}
							</p>
