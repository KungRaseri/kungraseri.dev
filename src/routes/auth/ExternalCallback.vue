...
export default class ExternalCallback extends Vue implements IApp {
	...
	public CallbackMessage: string = 'Thank you for logging in. You will be redirected shortly...';
		...
		this.$auth
			...
			.catch((err: any) => {
				if (err.error) {
					...
					this.CallbackMessage = 'Your login/registration request has failed for some reason.';
					setTimeout(() => {
						this.$router.push('/');
					}, 3000);
				}
